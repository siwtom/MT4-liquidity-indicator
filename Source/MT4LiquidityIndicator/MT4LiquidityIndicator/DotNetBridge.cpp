...
int CDotNetBridge::Execute(const string& assemblyPath, const string& typeName, const string& methodName, const string& argument /* = string */)
{
...
void CDotNetBridge::Shutdown()
{
	DWORD id = 0;
	HRESULT status = m_host->GetCurrentAppDomainId(&id);
	if (SUCCEEDED(status))
	{
		m_host->UnloadAppDomain(id, TRUE);
	}
}
